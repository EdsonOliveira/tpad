...
local  PUBLIC_PAD_STRING = S("Public (only owner's network)")
local  GLOBAL_PAD_STRING = S("Global (any network)")
...
function tpad.on_rightclick(clicked_pos, node, clicker)
	...
	if ownername == playername or minetest.get_player_privs(playername).tpad_admin then
		...
		padname_field:setLabel(S("This pad name (owned by ") .. OWNER_ESCAPE_COLOR .. ownername .. WHITE_ESCAPE .. ")")
...
local function forms_add_padlist(state, is_global)
	...
	local teleport_button = state:button(0.2, 7, 1.5, 0, "teleport_button", S("Teleport"))
	local close_button = state:button(6.5, 7, 1.5, 0, "close_button", S("Close"))
	close_button:setClose(true)	
	if is_global then
		local local_button = state:button(1.8, 7, 2.5, 0, "local_button", S("Local Network"))
		local_button:setClose(true)
	else
		local global_button = state:button(1.8, 7, 2.5, 0, "global_button", S("Global Network"))
		global_button:setClose(true)
	end
	state:label(0.2, 7.5, "teleport_label", S("(you can doubleclick on a pad to teleport)"))
end

tpad.forms.main_owner = smartfs.create("tpad.forms.main_owner", function(state)
	state:size(8, 8);
	state:field(0.5, 1, 6, 0, "padname_field", "", "")
	local save_button = state:button(6.5, 0.7, 1.5, 0, "save_button", S("Save"))
	...
	local delete_button = state:button(4.4, 7, 1.5, 0, "delete_button", S("Delete"))
